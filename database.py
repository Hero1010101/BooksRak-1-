...
class Book(Base):
    ...
    def recalculate_ratings(self):
        ratings = [self.rating_1, self.rating_2, self.rating_3, self.rating_4, self.rating_5]
        self.ratings_count = sum(ratings)
        self.avg_rating = sum(r * (idx + 1) for idx, r in enumerate(ratings)) / self.ratings_count if self.ratings_count > 0 else 0
...
def load_reviews_from_db(book_id):
    with engine.connect() as conn:
        ...
        if len(reviews) == 0:
            ...
            return [review._asdict() for review in reviews]  # Convert each SQLAlchemy RowProxy to a dictionary
